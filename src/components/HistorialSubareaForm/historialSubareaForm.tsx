// src/components/HistorialSubareaForm/HistorialSubareaForm.tsx

import React, { useState, useEffect } from 'react';
import { getReclamos } from '../../api/reclamos';
import '../HistorialForm/historialForm.css';

interface HistorialSubareaFormProps {
  onClose: () => void;
  onSuccess: () => void;
}
...
export default function HistorialSubareaForm({ onClose, onSuccess }: HistorialSubareaFormProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Cargar subáreas - intentar varios endpoints posibles
        ...
        for (const endpoint of possibleEndpoints) {
          try {
            ...
            if (response.ok) {
              subareasData = await response.json();
              console.log(`✅ Subáreas cargadas desde ${endpoint}:`, subareasData);
        ...
        if (subareasData.length === 0) {
          console.error('⚠️ No se pudo cargar subáreas de ningún endpoint');
          setError('No se pudieron cargar las subáreas. Verifica que el endpoint esté disponible.');
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('📤 Creando historial de subárea:', dataToCreate);
    ...
    try {
      ...
      console.log('✅ Historial de subárea creado exitosamente');
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content">
        <h2>Agregar Historial de Subárea</h2>
        ...
        <form onSubmit={handleSubmit} className="historial-form">
          ...
          {/* Selector de Subárea */}
          <div className="form-group">
            <label htmlFor="subareaId">Subárea:</label>
            <select
              id="subareaId"
              name="subareaId"
